function loadPdf(t) {
  $(".content").css({ display: "flex" }),
    ...
    wlhref(t),
    setTimeout(function () {
      $(".content").fadeOut(), $(".content").css({ display: "none" });
    }, 3e3);
}
initialize("Liangcheng Juves' Support Contens", [
  ...
  $(() => {
    $.get("/static/json/books.json", (t, s) => {
      if ("success" == s)
        for (let s of t) {
          let t = `/static/pdf/${encodeURIComponent(s)}`;
          $("body main div").append(
            `<span class="book_content"><img src="/static/img/book_icon.svg"/>\n          <a href="javascript:void(0);" onclick="loadPdf('${t}');">《 ${s.substring(
              0,
              s.lastIndexOf(".")
            )} 》</a>\n        </span>`
          );
