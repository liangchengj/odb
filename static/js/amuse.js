...
let afterMetaCharsetOf = (...eles) => {
  if (paramIsOK(eles)) {
    for (let m of metas) {
      ...
      if (paramIsOK(metaCharsetAttr) && "" != metaCharsetAttr) {
...
let jsof = (src, integrity, crossOrigin) => {
  let scriptAttr = { type: mediaTypeOf(".js"), src: src };
  if (paramIsOK(integrity)) {
    scriptAttr.integrity = integrity;
  }
  if (strContainChk(src, "//")) {
    scriptAttr.crossOrigin = paramIsOK(crossOrigin) ? crossOrigin : "anonymous";
  }
...
(() => {
  ...
  for (let m of metas) {
    ...
    if (paramIsOK(metaFaviconAttr) && "" != metaFaviconAttr) {
      iconHref = metaFaviconAttr;
    }
  }
